...
"${PACKCHK}" "${PACK_BUILD}/${PACK_VENDOR}.${PACK_NAME}.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/Keil.STM32L5xx_DFP.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/ARM.CMSIS.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/Keil.MDK-Middleware.pdsc" -n PackName.txt
errorlevel=$?
if [ $errorlevel -ne 0 ]; then
